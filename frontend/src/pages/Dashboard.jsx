...
export default function Dashboard() {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/api/socios').then(r => setSocios(r.data)).catch(()=>{})
    axios.get('http://localhost:3000/api/reservas').then(r => setReservas(r.data)).catch(()=>{})
  }, [])
    ...
    <div>
      ...
      <div className="cards-grid">
        <div className="metric-card">
          ...
          <div className="metric-value">{socios.length || 248}</div>
          <div className="metric-sub">+12 este mes</div>
        </div>
        <div className="metric-card">
          <div className="metric-label">📅 Reservas hoy</div>
          <div className="metric-value">{reservas.length || 34}</div>
          <div className="metric-sub">8 pendientes</div>
        </div>
        <div className="metric-card">
          <div className="metric-label">⏱ Uso promedio</div>
          <div className="metric-value">76<span style={{fontSize:'14px',color:'#555'}}>%</span></div>
          <div className="metric-sub">instalaciones</div>
        </div>
        <div className="metric-card">
          <div className="metric-label">💳 Membresías venc.</div>
          <div className="metric-value danger">7</div>
        ...
        <div className="dark-card">
          ...
          {RESERVAS_DEMO.map((r,idx) => (
            <div className="reserva-item" key={idx}>
              ...
              <span className={`badge badge-${r.estado}`}>{r.estado.charAt(0).toUpperCase()+r.estado.slice(1)}</span>
            </div>
          ))}
        </div>
      </div>
